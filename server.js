...
app.delete("/investimentos/:id", async (req, res) => {
  const { id } = req.params;
  console.log(`Tentando deletar investimento com ID: ${id}`); // Log do ID
  ...
  try {
    ...
    if (error || !data) {
      console.error("Investimento não encontrado:", error);
...
app.patch("/investimentos/reset", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Valor resetado com sucesso!", data });
  } catch (err) {
